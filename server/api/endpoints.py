...
def read_sensor_readings_endpoint(
    ...
    start_date: Optional[datetime] = Query(
        None, description="Fecha de inicio (formato ISO)"),
    end_date: Optional[datetime] = Query(
        None, description="Fecha de fin (formato ISO)"),
    db: Session = Depends(get_db)
):
    print(
        f"🌐 HTTP GET /readings/ | Parámetros: skip={skip}, limit={limit}, start_date={start_date}, end_date={end_date}")
...
def read_latest_sensor_reading_endpoint(db: Session = Depends(get_db)):
    ...
    if db_reading is None:
        raise HTTPException(
            status_code=404, detail="No hay lecturas disponibles")
    return db_reading
...
def read_watering_events_endpoint(
    ...
    print(
        f"🌐 HTTP GET /watering-events/ | Parámetros: skip={skip}, limit={limit}, start_date={start_date}, end_date={end_date}")
    events = crud.get_watering_events(
        db, skip=skip, limit=limit, start_date=start_date, end_date=end_date)
    return events
...
def get_stats_in_range(
    ...
    print(
        f"🌐 HTTP GET /stats/ | Parámetros: start_date={start_date}, end_date={end_date}")
    readings = crud.get_sensor_readings(
        db, limit=10000, start_date=start_date, end_date=end_date)
    ...
    stats = {
        ...
        "humidity": {"average": round(avg_humidity, 2), "max": max_humidity, "min": min_humidity},
        "light": {"average": round(avg_light, 2)}
    }
